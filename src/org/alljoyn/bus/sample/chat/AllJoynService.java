...
public class AllJoynService extends Service implements Observer {
    ...
    private void doStartDiscovery() {
    	...
    	/*String MASTER_CHANNEL = "MASTER_CHANNEL";
    	
    	int attemptCount = 0;
    	while (attemptCount++ < 5)
    	{
    		Log.i(TAG, "Looping and looking for channel");
    		if (mChatApplication.getFoundChannels().contains(MASTER_CHANNEL))
            {
            	Log.i(TAG, "Joining master channel");
            	mChatApplication.useSetChannelName(MASTER_CHANNEL);
        ...
        // Else create channel
    	Log.i(TAG, "Creating new channel");
    	
    	mChatApplication.hostSetChannelName(MASTER_CHANNEL);
    	
    	mChatApplication.hostInitChannel();
    	try { Thread.sleep(1000); } catch(Exception e) {};
    	mChatApplication.hostStartChannel();
    	
    	mChatApplication.getFoundChannels();
