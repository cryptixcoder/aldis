...
public class ChatApplication extends Application implements Observable {
    ...
    private static final String NEW_TO_CHANNEL = "NEWTOCHANNEL";
    private static final String MASTER_CHANNEL = "MASTER_CHANNEL";
    ...
    public void onCreate() {
        ...
        this.hostSetChannelName(MASTER_CHANNEL);
		this.hostInitChannel();
	...
	public synchronized void useJoinChannel() {
		...
		Log.i(TAG, "NEW TO CHANNEL");
		addOutboundItem(NEW_TO_CHANNEL);
		for (Message m: messageDAO.getAllMessages())
		{
			addOutboundItem(m.getMessage());
		}
	}
	...
	public synchronized void newLocalUserMessage(String message) {
		if (!mHistory.containsKey(message))
			addInboundItem("Me", mNickname + " - " + message);

		if (useGetChannelState() == AllJoynService.UseChannelState.JOINED) {
			addOutboundItem(mNickname + " - " + message);
		}
	}
	...
	{		
		if (message.contains(NEW_TO_CHANNEL)) // if new user joined channel
		{
			Log.i(TAG, nickname + " joined channel" );
			for (Message m  : messageDAO.getAllMessages())
			{	// Flood our saved messages
				addOutboundItem(m.getMessage());
			}			
		}		
		else 
	...
	private void addInboundItem(String nickname, String message) {
		...
		if (!message.contains(NEW_TO_CHANNEL))
		{
			if(!mHistory.containsKey(message))
			{
				messageDAO.createMessage(message);
				mHistory.put(message, new Date().getTime());
			}
		}
		
		else	// inbound user is new to channel
		{
			//if (!mHistory.containsKey(message))
			//{
				for (Message m  : messageDAO.getAllMessages())
	...
	private HashMap<String, Long> mHistory = new HashMap<String, Long>(MySQLiteHelper.MESSAGE_LIMIT);
	/**
	 ...
	 * the local time at which the message was received, the nickname of the 
    ...
    public synchronized List<String> getHistory() {
        ...
        //}
        //return clone;
    	return messageDAO.getAllMessageStrings();
    }
